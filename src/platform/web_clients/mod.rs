...
impl Connectors {
    pub fn new(key: &str, secret: &str, is_live: bool, shutdown_signal: CancellationToken) -> Result<Arc<Self>> {
    ...
    pub async fn get_account_details(&self) -> Result<account::Account> {
        self.http_client.send_request::<account::Get>(&self.client, &()).await
    }

    pub async fn get_orders(&self) -> Result<Vec<order::Order>> {
        let request = orders::OrdersReq::default();
        self.http_client.send_request::<orders::Get>(&self.client, &request).await
    }

    pub async fn get_positions(&self) -> Result<Vec<position::Position>> {
        self.http_client.send_request::<positions::Get>(&self.client, &()).await
    }

    pub async fn place_order(&self, request: &order::OrderReq) -> Result<order::Order> {
        self.http_client.send_request::<order::Post>(&self.client, request).await
    }

    pub async fn cancel_order(&self, id: &order::Id) -> Result<()> {
        self.http_client.send_request::<order::Delete>(&self.client, id).await
    }

    pub async fn close_position(&self, symbol: &asset::Symbol) -> Result<order::Order> {
        self.http_client.send_request::<position::Delete>(&self.client, &symbol).await
    }

    pub async fn get_historical_bars(&self, request: &bars::BarsReq) -> Result<bars::Bars> {
        self.http_client.send_request::<bars::Get>(&self.client, &request).await
    }
