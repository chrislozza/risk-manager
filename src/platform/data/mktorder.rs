...
impl MktOrder {
    ...
    async fn persist_db(&mut self, db: Arc<DBClient>, override_id: Option<Uuid>) -> Result<()> {
        let columns = vec![
            "action",
            "strategy",
            "symbol",
        ...
        if Uuid::is_nil(&self.local_id) {
            self.local_id = match override_id {
...
impl MktOrders {
    ...
    pub async fn startup(&mut self, order_ids: Vec<Uuid>) -> Result<Vec<&MktOrder>> {
        ...
        let mktorders = Vec::from_iter(self.mktorders.values().into_iter());
        Ok(mktorders)
    }
    ...
    pub async fn add_order(
        ...
        mktorder.persist_db(self.db.clone(), None).await?;
